...
export default async (app, { getRouter }) => {
    ...
    app.on(
        ...
        async (context) => {
            ...
            for (const repository of repositories) {
                ...
                const emptyRepo = await isRepoEmpty(
                    context,
                    owner,
                    repository.name
                )
                ...
                if (!emptyRepo) {
                    ...
                    latestCommitInfo.latest_commit_sha =
    ...
    app.on('issues.edited', async (context) => {
        ...
        if (issueBody.includes('<!-- @codefair-bot rerun-cwl-validation -->')) {
            try {
                ...
                const issueBodyRemovedCommand = issueBody.substring(
                    0,
                    issueBody.indexOf(
                        `<sub><span style="color: grey;">Last updated`
                    )
                )
            ...
            try {
                ...
                if (cwlExists?.contains_cwl_files) {
                    ...
                    if (cwlExists.files.length > 0) {
                        ...
                        const cwlFilePaths = cwlObject.files.map(
                            (file) => file.path
                        )
                        cwlObject.removed_files = cwlExists.files.filter(
                            (file) => {
                ...
                if (error.cause) {
                    logwatch.error(
                        {
                            message:
                                'Error.cause message for Full Repo Validation',
                            error: error.cause,
                        },
                        true
                    )
        ...
        if (
            issueBody.includes(
                '<!-- @codefair-bot rerun-license-validation -->'
            )
        ) {
            ...
            try {
                ...
                if (existingLicense) {
                    await db.licenseRequest.update({
                        data: {
                            ...
                            license_status: licenseContentEmpty
                                ? 'valid'
                                : 'invalid',
                        },
                        where: {
                ...
                if (error.cause) {
                    logwatch.error(
                        {
                            message:
                                'Error.cause message for License Validation',
                            error: error.cause,
                        },
                        true
                    )
        ...
        if (
            issueBody.includes(
                '<!-- @codefair-bot rerun-metadata-validation -->'
            )
        ) {
            ...
            try {
                ...
                const citation = await getCitationContent(
                    context,
                    owner,
                    repository
                )
                const codemeta = await getCodemetaContent(
                    context,
                    owner,
                    repository
                )
                ...
                if (metadata.creationDate) {
                    metadata.creationDate = convertDateToUnix(
                ...
                if (existingMetadataEntry) {
                    await db.codeMetadata.update({
                        data: {
                            codemeta_status: validCodemeta
                                ? 'valid'
                                : 'invalid',
                            citation_status: validCitation
                                ? 'valid'
                                : 'invalid',
                ...
                if (error.cause) {
                    logwatch.error(
                        {
                            message:
                                'Error.cause message for Metadata Validation',
                            error: error.cause,
                        },
                        true
                    )
        ...
        if (issueBody.includes('<!-- @codefair-bot publish-zenodo')) {
            ...
            try {
                ...
                const citationCff = await getCitationContent(
                    context,
                    owner,
                    repository
                )
                ...
                const zenodoDoi =
                    zenodoDepositionInfo.metadata.prereserve_doi.doi
                ...
                const finalTempString =
                    await applyLastModifiedTemplate(tempString)
                await createIssue(
                    context,
                    owner,
                    repository,
                ...
                if (error.cause) {
                    logwatch.error(
                        {
                            message:
                                'Error.cause message for Zenodo Publishing',
                            error: error.cause,
                        },
                        true
                    )
                ...
                throw new Error(
                    `Error publishing to Zenodo: ${error.message}`,
                    { cause: error }
                )
            }
        }
